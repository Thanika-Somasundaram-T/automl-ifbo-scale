...
check_flops_sorted("./all_curves.json")
...
def count_curves_per_target(path):
    with open(path, "r") as f:
        data = json.load(f)
    ...
    for run_key, run_data in data.items():
        ...
        # FIX: correct path
        target = run_data.get("hyperparameters", {}).get("target_N", None)
    ...
    print("Curves per target_N:\n")

    for target, count in sorted(target_counts.items(), key=lambda x: x[1], reverse=True):
        print(f"target_N={target}: {count}")

    print(f"\nTotal runs counted: {sum(target_counts.values())}")
    print(f"Total runs in file: {len(data)}")

    return dict(target_counts)

count_curves_per_target("./all_curves.json")