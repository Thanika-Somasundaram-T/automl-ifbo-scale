"""Experiment 0.13 — bracket_nearest supervisor-style figure.

Same PAWS HP slice as Ex0.6/Ex0.7, but each query target gets the nearest
lower and nearest upper full curves as context plus its own observed prefix.
"""
...
DEFAULT_OUTPUT_DIR = Path("experiments/ex0_13_bracket_nearest")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ex0.13: bracket_nearest supervisor figure.")
...
def predict_bracket_nearest(
    ...
    for target_n in targets:
        lower = [partner_n for partner_n in targets if partner_n < target_n]
        upper = [partner_n for partner_n in targets if partner_n > target_n]
        if not lower or not upper:
            continue
        nearest_below = max(lower)
        nearest_above = min(upper)
        partners = [nearest_below, nearest_above]
        ...
        rows.append(pd.DataFrame({
            "policy": "bracket_nearest",
            "target_N": int(target_n),
            "obs_frac": float(obs_frac),
            "context_size": len(context),
            "n_partners": 2,
            "nearest_below": int(nearest_below),
            "nearest_above": int(nearest_above),
    ...
    if not rows:
        raise RuntimeError("No targets had bracket partners and query points")
...
def uncertainty_diagnostics(predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    for target_n, group in predictions.groupby("target_N", sort=True):
        ...
        records.append({
            "target_N": int(target_n),
            "n_partners": int(ordered["n_partners"].iloc[0]),
            "nearest_below": int(ordered["nearest_below"].iloc[0]),
            "nearest_above": int(ordered["nearest_above"].iloc[0]),
...
def plot_supervisor(
    ...
    *,
    ...
    for i, target_n in enumerate(targets):
        ...
        target_handles[
            f"{label} (bracket {int(pred['nearest_below'].iloc[0]) / 1e6:.0f}M/"
            f"{int(pred['nearest_above'].iloc[0]) / 1e6:.0f}M)"
        ] = obs_line
    ...
    axes[1].set_title("bracket_nearest predictions + p05-p95 bands")
    for ax in axes:
    ...
    fig.suptitle(
        f"Ex0.13 bracket_nearest FT-PFN — base_N={base_n / 1e6:.2f}M, "
...
def plot_separate_curves(
    ...
    for target_n in sorted(int(target_n) for target_n in predictions["target_N"].unique()):
        ...
        ax.set_title(f"Ex0.13 bracket_nearest — target_N={target_n}, obs={obs_frac:.2f}")
        ax.grid(True, alpha=0.3)
...
def plot_cascade_curves(
    ...
    fig.suptitle("Ex0.13 bracket_nearest", fontsize=12)
    fig.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_path, dpi=170, bbox_inches="tight")
    plt.close(fig)
...
def write_notes(
    ...
    *,
    ...
    output_dir.joinpath("SUMMARY.md").write_text(
        f"""# Ex0.13 — bracket_nearest FT-PFN Figure
...
def main() -> None:
    ...
    for obs_frac in args.obs_fracs:
        ...
        skipped_targets_by_obs_frac[f"{obs_frac:.2f}"] = skipped_targets

        print(f"[bracket_nearest obs={obs_frac:.2f}] skipped targets: {skipped_targets}")
        shard = partial_dir / f"bracket_nearest__obs{shard_tag(obs_frac)}.parquet"
        ...
        plot_supervisor(
            ...
            args.output_dir / "plots" / obs_dir / "supervisor_styles" / "bracket_nearest_supervisor_style.png",
            base_n=args.base_n,
            shrink=args.shrink,
            tkpm=args.tkpm,
        )
        ...
        plot_cascade_curves(
            ...
            obs_frac,
            (float(args.ylim[0]), float(args.ylim[1])),
            args.output_dir / "plots" / obs_dir / "cascade_styles" / "bracket_nearest_cascade_curves.png",
        )
    ...
    merge_shards(partial_dir, results_dir, "bracket_nearest_predictions.parquet")
    diagnostics_all = pd.concat(all_diagnostics, ignore_index=True)
    diagnostics_all.to_csv(results_dir / "uncertainty_diagnostics.csv", index=False)

    merged_predictions = pd.read_parquet(results_dir / "bracket_nearest_predictions.parquet")
    summary = {
        "experiment": "ex0_13_bracket_nearest",
        ...
        "predicted_targets": sorted(int(target_n) for target_n in merged_predictions["target_N"].unique()),
        "skipped_targets": last_skipped_targets,
        "skipped_targets_by_obs_frac": skipped_targets_by_obs_frac,
        "n_prediction_rows": int(len(merged_predictions)),
        "seed": args.seed,
        "device": str(device),
        ...
        "context_size_by_target": {
            ...
            for target_n, size in merged_predictions.groupby("target_N")["context_size"].first().items()
        },
        "partners_by_target": {
            str(int(target_n)): {
                "nearest_below": int(group["nearest_below"].iloc[0]),
                "nearest_above": int(group["nearest_above"].iloc[0]),
            }
            for target_n, group in merged_predictions.groupby("target_N")
        },
    }
