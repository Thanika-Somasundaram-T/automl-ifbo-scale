"""Experiment 0.10 — lower_final_pt_only supervisor-style figure.

Same PAWS HP slice as Ex0.6-0.9, but each query target gets only the
final point from every strictly lower-scale curve plus its own observed prefix.
"""
...
DEFAULT_OUTPUT_DIR = Path("experiments/ex0_10_lower_final_pt_only")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ex0.10: lower_final_pt_only supervisor figure.")
...
def predict_lower_final_pt_only(
    model: FTPFN,
    ...
    for target_n in targets:
        ...
        for partner_n in lower_targets:
            ...
            final_row = partner_df.iloc[-1]
            pt = torch.tensor([float(final_row["x_norm"])], dtype=torch.float32)
            py_raw = torch.tensor([float(final_row["Validation Loss"])], dtype=torch.float32)
...
def plot_supervisor(
    ...
    *,
    ...
    for i, target_n in enumerate(targets):
        ...
        target_handles[f"{label} ({int(pred['n_lower_partners'].iloc[0])} final pts)"] = obs_line
    ...
    axes[1].set_title("lower_final_pt_only predictions + p05-p95 bands")
    for ax in axes:
    ...
    fig.suptitle(
        f"Ex0.10 lower_final_pt_only FT-PFN — base_N={base_n / 1e6:.2f}M, "
...
def plot_cascade_curves(
    ...
    fig.suptitle("Ex0.10 lower_final_pt_only", fontsize=12)
    fig.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_path, dpi=170, bbox_inches="tight")
    plt.close(fig)
...
def write_notes(
    ...
    *,
    ...
    output_dir.joinpath("SUMMARY.md").write_text(
        f"""# Ex0.10 — lower_final_pt_only FT-PFN Figure
...
def main() -> None:
    ...
    for obs_frac in args.obs_fracs:
        predictions = predict_lower_final_pt_only(model, curves, obs_frac, hp_map)
        ...
        shard = partial_dir / f"lower_final_pt_only__obs{shard_tag(obs_frac)}.parquet"
        ...
        plot_supervisor(
            ...
            args.output_dir / "plots" / obs_dir / "supervisor_styles" / "lower_final_pt_only_supervisor_style.png",
            base_n=args.base_n,
            shrink=args.shrink,
            tkpm=args.tkpm,
        )
        plot_cascade_curves(
            ...
            obs_frac,
            (float(args.ylim[0]), float(args.ylim[1])),
            args.output_dir / "plots" / obs_dir / "cascade_styles" / "lower_final_pt_only_cascade_curves.png",
        )
    ...
    merge_shards(partial_dir, results_dir, "lower_final_pt_only_predictions.parquet")

    summary = {
        "experiment": "ex0_10_lower_final_pt_only",
        ...
        "n_prediction_rows": int(len(pd.read_parquet(results_dir / "lower_final_pt_only_predictions.parquet"))),
        "seed": args.seed,
        "device": str(device),
