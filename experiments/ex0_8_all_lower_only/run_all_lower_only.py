"""Experiment 0.8 — all_lower_only supervisor-style figure.

Same PAWS HP slice as Ex0.6/Ex0.7, but each query target gets only
strictly lower-scale full curves as context plus its own observed prefix.
"""
...
DEFAULT_OUTPUT_DIR = Path("experiments/ex0_8_all_lower_only")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ex0.8: all_lower_only supervisor figure.")
...
def plot_supervisor(
    ...
    *,
    ...
    for i, target_n in enumerate(targets):
        ...
        target_handles[f"{label} ({int(pred['n_lower_partners'].iloc[0])} lower)"] = obs_line
    ...
    axes[1].set_title("all_lower_only predictions + p05-p95 bands")
    for ax in axes:
    ...
    fig.suptitle(
        f"Ex0.8 all_lower_only FT-PFN — base_N={base_n / 1e6:.2f}M, "
...
def plot_separate_curves(
    ...
    for target_n in sorted(int(target_n) for target_n in predictions["target_N"].unique()):
        ...
        ax.set_title(f"Ex0.8 all_lower_only — target_N={target_n}, obs={obs_frac:.2f}")
        ax.grid(True, alpha=0.3)
...
def plot_cascade_curves(
    ...
    fig.suptitle("Ex0.8 all_lower_only", fontsize=12)
    fig.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_path, dpi=170, bbox_inches="tight")
    plt.close(fig)
...
def write_notes(
    ...
    *,
    ...
    output_dir.joinpath("SUMMARY.md").write_text(
        f"""# Ex0.8 — all_lower_only FT-PFN Figure
...
def main() -> None:
    ...
    for obs_frac in args.obs_fracs:
        predictions = predict_all_lower_only(model, curves, obs_frac, hp_map)
        ...
        shard = partial_dir / f"all_lower_only__obs{shard_tag(obs_frac)}.parquet"
        ...
        plot_supervisor(
            ...
            args.output_dir / "plots" / obs_dir / "supervisor_styles" / "all_lower_only_supervisor_style.png",
            base_n=args.base_n,
            shrink=args.shrink,
            tkpm=args.tkpm,
        )
        ...
        plot_cascade_curves(
            ...
            obs_frac,
            (float(args.ylim[0]), float(args.ylim[1])),
            args.output_dir / "plots" / obs_dir / "cascade_styles" / "all_lower_only_cascade_curves.png",
        )
    ...
    merge_shards(partial_dir, results_dir, "all_lower_only_predictions.parquet")

    summary = {
        "experiment": "ex0_8_all_lower_only",
        ...
        "n_prediction_rows": int(len(pd.read_parquet(results_dir / "all_lower_only_predictions.parquet"))),
        "seed": args.seed,
        "device": str(device),
