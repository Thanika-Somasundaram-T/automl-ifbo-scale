"""Experiment 0.9 — nearest_lower_only supervisor-style figure.

Same PAWS HP slice as Ex0.6-0.8, but each query target gets only the
nearest lower-scale full curve as context plus its own observed prefix.
"""
...
DEFAULT_OUTPUT_DIR = Path("experiments/ex0_9_nearest_lower_only")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ex0.9: nearest_lower_only supervisor figure.")
...
def predict_nearest_lower_only(
    model: FTPFN,
    ...
    for target_n in targets:
        ...
        if not lower_targets:
            continue
        nearest_partner = max(lower_targets)
        ...
        partner_df = curves[nearest_partner]
        ...
        context.append(Curve(hyperparameters=hp_map[nearest_partner], t=pt, y=partner_perf))
        ...
        rows.append(pd.DataFrame({
            "policy": "nearest_lower_only",
            "target_N": int(target_n),
            "obs_frac": float(obs_frac),
            "context_size": len(context),
            "nearest_partner_target_N": int(nearest_partner),
    ...
    if not rows:
        raise RuntimeError("No targets had nearest lower partners and query points")
...
def uncertainty_diagnostics(predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    for target_n, group in predictions.groupby("target_N", sort=True):
        ...
        records.append({
            "target_N": int(target_n),
            "nearest_partner_target_N": int(ordered["nearest_partner_target_N"].iloc[0]),
...
def plot_supervisor(
    ...
    *,
    ...
    for i, target_n in enumerate(targets):
        ...
        nearest = int(pred["nearest_partner_target_N"].iloc[0] / 1e6)
        target_handles[f"{label} (nearest {nearest}M)"] = obs_line
    ...
    axes[1].set_title("nearest_lower_only predictions + p05-p95 bands")
    for ax in axes:
    ...
    fig.suptitle(
        f"Ex0.9 nearest_lower_only FT-PFN — base_N={base_n / 1e6:.2f}M, "
...
def plot_separate_curves(
    ...
    for target_n in sorted(int(target_n) for target_n in predictions["target_N"].unique()):
        ...
        ax.set_title(f"Ex0.9 nearest_lower_only — target_N={target_n}, obs={obs_frac:.2f}")
        ax.grid(True, alpha=0.3)
...
def plot_cascade_curves(
    ...
    fig.suptitle("Ex0.9 nearest_lower_only", fontsize=12)
    fig.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_path, dpi=170, bbox_inches="tight")
    plt.close(fig)
...
def write_notes(
    ...
    *,
    ...
    output_dir.joinpath("SUMMARY.md").write_text(
        f"""# Ex0.9 — nearest_lower_only FT-PFN Figure
...
def main() -> None:
    ...
    for obs_frac in args.obs_fracs:
        predictions = predict_nearest_lower_only(model, curves, obs_frac, hp_map)
        ...
        shard = partial_dir / f"nearest_lower_only__obs{shard_tag(obs_frac)}.parquet"
        ...
        plot_supervisor(
            ...
            args.output_dir / "plots" / obs_dir / "supervisor_styles" / "nearest_lower_only_supervisor_style.png",
            base_n=args.base_n,
            shrink=args.shrink,
            tkpm=args.tkpm,
        )
        ...
        plot_cascade_curves(
            ...
            obs_frac,
            (float(args.ylim[0]), float(args.ylim[1])),
            args.output_dir / "plots" / obs_dir / "cascade_styles" / "nearest_lower_only_cascade_curves.png",
        )
    ...
    merge_shards(partial_dir, results_dir, "nearest_lower_only_predictions.parquet")

    summary = {
        "experiment": "ex0_9_nearest_lower_only",
        ...
        "n_prediction_rows": int(len(pd.read_parquet(results_dir / "nearest_lower_only_predictions.parquet"))),
        "seed": args.seed,
        "device": str(device),
        ...
        "nearest_partner_target_N_by_target": {
            str(int(target_n)): int(partner)
            for target_n, partner in last_predictions.groupby("target_N")["nearest_partner_target_N"].first().items()
        },
    }
