"""Experiment 0.12 — lower_start_final_pt supervisor-style figure.

Same PAWS HP slice as Ex0.6-0.11, but each query target gets every
strictly lower-scale curve reduced to start and final points plus its own
observed prefix.
"""
...
DEFAULT_OUTPUT_DIR = Path("experiments/ex0_12_lower_start_final_pt")
...
POINTS_PER_PARTNER = 2
SKIPPED_TARGETS = {32270848}
EPS = 1e-12


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ex0.12: lower_start_final_pt supervisor figure.")
...
def start_final_indices(n_points: int) -> np.ndarray:
    if n_points < POINTS_PER_PARTNER:
        raise ValueError(f"Expected at least {POINTS_PER_PARTNER} partner points, got {n_points}")
    return np.array([0, n_points - 1], dtype=int)
...
def predict_lower_start_final_pt(
    model: FTPFN,
    ...
    for target_n in targets:
        ...
        for partner_n in lower_targets:
            ...
            idx = start_final_indices(len(partner_df))
...
def plot_supervisor(
    ...
    *,
    ...
    for i, target_n in enumerate(targets):
        ...
        target_handles[f"{label} ({int(pred['n_lower_partners'].iloc[0])}×2 pts)"] = obs_line
    ...
    axes[1].set_title("lower_start_final_pt predictions + p05-p95 bands")
    for ax in axes:
    ...
    fig.suptitle(
        f"Ex0.12 lower_start_final_pt FT-PFN — base_N={base_n / 1e6:.2f}M, "
...
def plot_cascade_curves(
    ...
    fig.suptitle("Ex0.12 lower_start_final_pt", fontsize=12)
    fig.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_path, dpi=170, bbox_inches="tight")
    plt.close(fig)
...
def write_notes(
    ...
    *,
    ...
    output_dir.joinpath("SUMMARY.md").write_text(
        f"""# Ex0.12 — lower_start_final_pt FT-PFN Figure
...
def main() -> None:
    ...
    for obs_frac in args.obs_fracs:
        predictions = predict_lower_start_final_pt(model, curves, obs_frac, hp_map)
        ...
        shard = partial_dir / f"lower_start_final_pt__obs{shard_tag(obs_frac)}.parquet"
        ...
        plot_supervisor(
            ...
            args.output_dir / "plots" / obs_dir / "supervisor_styles" / "lower_start_final_pt_supervisor_style.png",
            base_n=args.base_n,
            shrink=args.shrink,
            tkpm=args.tkpm,
        )
        plot_cascade_curves(
            ...
            obs_frac,
            (float(args.ylim[0]), float(args.ylim[1])),
            args.output_dir / "plots" / obs_dir / "cascade_styles" / "lower_start_final_pt_cascade_curves.png",
        )
    ...
    merge_shards(partial_dir, results_dir, "lower_start_final_pt_predictions.parquet")

    summary = {
        "experiment": "ex0_12_lower_start_final_pt",
        ...
        "n_prediction_rows": int(len(pd.read_parquet(results_dir / "lower_start_final_pt_predictions.parquet"))),
        "seed": args.seed,
        "device": str(device),
